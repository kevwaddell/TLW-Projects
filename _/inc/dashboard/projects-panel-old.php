...
<div class="panel panel-default">
	<div class="panel-heading">
		<h3 class="panel-title text-center">Recent Projects</h3>
	</div>
		...
		<div class="btn-group filter-actions">
			...
			<div class="table-responsive">
				<table class="table table-bordered">
					...
					$date = get_field('project_start_date',$p->ID);	
					$created_by = get_userdata($p->post_author);	
					$end_date = get_field('project_completed_date',$p->ID);	
					$company = wp_get_post_terms($p->ID, 'tlw_company_tax', array("fields" => "names"));
					$media_type = get_the_term_list($p->ID, 'tlw_media_types', '', ', ');
					$tasks_args = array(
					'post_type'	=> 'tlw_task',
					'posts_per_page' => -1
					);
					if ($end_date) {
					$tasks_args['meta_query'] = array('relation' => 'AND',array('key'	=> 'project','value'	=> $p->ID), array('key'	=> 'task_status','value'	=> 'completed'));
					} else {
					$tasks_args['meta_query'] = array('relation' => 'AND',array('key'	=> 'project','value'	=> $p->ID), array('key'	=> 'task_status','value'	=> 'pending'));	
					}
					...
					<tr class="<?php echo ($end_date) ? 'success':'danger'; ?>">
						...
						<td>
							<p class="bold"><i class="fa <?php echo ($end_date) ? 'fa-check-circle col-success':'fa-clock-o col-danger'; ?>"></i> <?php echo $title; ?></p>
						...
						<td>
							<a href="<?php echo get_permalink($p->ID); ?>" class="visible-xs btn btn-<?php echo ($end_date) ? 'success':'danger'; ?>"><i class="fa fa-angle-right"></i></a>
							...
							<ul class="dropdown-menu" role="menu">
								...
								<li><a href="?request=complete-project&pid=<?php echo $p->ID; ?>" class="request-btn"><i class="fa fa-check"></i> Complete Project</a></li>
								<?php }  ?>
								<li><a href="?request=edit-project&pid=<?php echo $p->ID; ?><?php echo ($end_date) ? '&projects-filter=complete':''; ?>" class="request-btn"><i class="fa fa-pencil"></i> Edit Project</a></li>
								<li><a href="?request=delete-project&pid=<?php echo $p->ID; ?><?php echo ($end_date) ? '&projects-filter=complete':''; ?>" class="request-btn"><i class="fa fa-trash"></i> Delete Project</a></li>
								<li><a href="?request=add-task&pid=<?php echo $p->ID; ?><?php echo ($end_date) ? '&projects-filter=complete':''; ?>" class="request-btn"><i class="fa fa-plus"></i> Add Task</a></li>
							</ul>
						</div>
						</td>
					</tr>
